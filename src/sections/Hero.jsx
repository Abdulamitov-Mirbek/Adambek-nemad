...
const LanguageSwitcher = ({ language, setLanguage, isVisible }) => {
  return (
    <motion.div
      ...
      className="fixed top-4 sm:top-6 right-4 sm:right-6 z-[100] flex items-center bg-black/60 backdrop-blur-md border border-white/10 rounded-full p-1 shadow-sm sm:p-1.5 sm:shadow-2xl"
      style={{ pointerEvents: isVisible ? "auto" : "none" }}
    >
      <button
        onClick={() => setLanguage("ru")}
        aria-label="Switch to Russian"
        className={`relative px-3 sm:px-5 py-1 sm:py-1.5 rounded-full text-[10px] sm:text-xs font-bold tracking-widest transition-all duration-300 ${
          language === "ru"
...
export const Hero = () => {
  ...
  return (
    ...
    >
      ...
      <div className="container px-4 sm:px-6 relative z-20 text-center">
        ...
        <motion.h1
          ...
          className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-black mb-4 sm:mb-6 md:mb-8 leading-[1.1] sm:leading-[1] tracking-tighter"
        >
          {t.title}
          <br />
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 whitespace-nowrap inline-block">
            {t.titleGradient}
          </span>
        </motion.h1>
        ...
        >
          <a
            ...
            className="w-full sm:w-auto px-6 sm:px-8 md:px-10 py-3 sm:py-4 bg-white text-black rounded-full font-bold text-sm sm:text-base hover:bg-blue-600 hover:text-white transition-all duration-300 shadow-[0_0_20px_rgba(255,255,255,0.1)] hover:shadow-blue-500/40 text-center inline-block"
          >
            {t.button1}
          </a>
          <button
            ...
            className="w-full sm:w-auto px-6 sm:px-8 md:px-10 py-3 sm:py-4 border border-white/20 rounded-full font-bold text-sm sm:text-base hover:bg-white/10 backdrop-blur-sm transition-all duration-300 text-center inline-block cursor-pointer"
          >
            {t.button2}
          </button>
        </motion.div>
      ...
      <motion.div
        ...
        className="absolute bottom-6 sm:bottom-10 left-1/2 -translate-x-1/2 opacity-30"
      >
