...
// Импорты ассетов (оставляем как есть)
...
const resultsData = {
  ...
  kg: [
    ...
    { name: "Кутман Нурланбекович", role: "Эксперт / Сатуу", result: "Жеке бренд", description: "Жогорку чекке чыгуу и аудиториянын сапатын жогорулатуу.", img: kutman_nurlanbekovich, handle: "@kutman_nurlanbekovich", insta: "https://www.instagram.com/kutman_nurlanbekovich/" },
...
function StoriesSection({ id, title, results, t, bgClass }) {
  ...
  const handleScroll = () => {
    ...
    if (!item) return;
    const itemWidth = item.clientWidth + 24; // ширина + gap
  ...
  useEffect(() => {
    const container = scrollRef.current;
    if (!container) return;
    const checkScroll = () => setCanScroll(container.scrollWidth > container.clientWidth);
    ...
    <section id={id} className={`scroll-mt-24 py-24 overflow-hidden ${bgClass}`}>
      <div className="max-w-7xl mx-auto px-6">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
          <span className="text-[10px] font-black tracking-[0.3em] text-blue-600 uppercase bg-blue-50 px-3 py-1 rounded-full">SUCCESS STORIES</span>
          <h2 className="text-4xl md:text-5xl font-black text-gray-900 mt-4 tracking-tighter uppercase leading-none">{title}</h2>
        </motion.div>
        ...
        <div className="relative">
              ...
              >
                ...
                >
                  <div className="relative aspect-[7/9] overflow-hidden">
                    ...
                    {item.isGold && <div className="absolute top-5 left-5 bg-yellow-400 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase">Best Case</div>}
                  </div>
          ...
          {/* Индикатор отображается всегда при наличии скролла на мобильных */}
          {canScroll && isMobile && (
        ...
        {results.length > 3 && (
          <div className="flex justify-center mt-10">
            <button
              ...
              className={`group inline-flex items-center gap-2 px-8 py-3 rounded-full font-bold text-sm shadow-lg transition-all duration-300 ${
                showAll ? "bg-gray-100 text-gray-600" : "bg-gradient-to-r from-blue-600 to-purple-600 text-white hover:scale-105"
              }`}
            >
...
export const Awards = () => {
  ...
  return (
    <>
      <StoriesSection id="students" title={t.studentTitle} results={students} t={t} bgClass="bg-white" />
      <StoriesSection id="enterprises" title={t.enterpriseTitle} results={enterprises} t={t} bgClass="bg-gray-50" />
    </>
  );
};
